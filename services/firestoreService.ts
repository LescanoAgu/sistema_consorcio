...
// AUTOMATIZACIÓN DE FONDO DE RESERVA
export const rebuildReserveFund = async (consortiumId: string, currentPercentage: number) => {
    const batch = writeBatch(db);
    
    // 1. Obtener todas las transacciones actuales
    const txSnapshot = await getDocs(collection(db, `consortiums/${consortiumId}/reserve_transactions`));
    const oldTx = txSnapshot.docs.map(d => ({ id: d.id, ...d.data() } as ReserveTransaction));
    
    // 2. Obtener todos los cobros (payments aprobados) y todos los gastos (expenses)
    const paymentsSnap = await getDocs(query(collection(db, `consortiums/${consortiumId}/payments`), where('status', '==', 'APPROVED')));
    const expensesSnap = await getDocs(query(collection(db, `consortiums/${consortiumId}/expenses`), where('distributionType', '==', 'FROM_RESERVE')));
    
    // 3. Borrar transacciones 'SYSTEM' viejas
    oldTx.filter(t => t.type === 'SYSTEM').forEach(t => {
        batch.delete(doc(db, `consortiums/${consortiumId}/reserve_transactions`, t.id));
    });
    
    // 4. Calcular nuevos ingresos (Pagos)
    let runningBalance = 0;
    const newTransactions: Omit<ReserveTransaction, 'id'>[] = [];
    
    paymentsSnap.docs.forEach(doc => {
        const p = doc.data() as Payment;
        const amountForReserve = p.amount * (currentPercentage / 100);
        if (amountForReserve > 0) {
            newTransactions.push({
                date: p.date,
                amount: amountForReserve,
                description: `Aporte de Cobro (${doc.id.slice(-4)})`,
                type: 'SYSTEM'
            });
        }
    });
    
    // 5. Calcular nuevos egresos (Gastos de Reserva)
    expensesSnap.docs.forEach(doc => {
        ...
        if (e.amount > 0) {
            newTransactions.push({
                date: e.date,
                amount: -e.amount,
                description: `Pago de Gasto: ${e.description}`,
                type: 'SYSTEM'
            });
        }
    });
    
    // 6. Insertar nuevas transacciones y sumar balance total
    newTransactions.forEach(tx => {
        const newRef = doc(collection(db, `consortiums/${consortiumId}/reserve_transactions`));
        batch.set(newRef, tx);
        runningBalance += tx.amount;
    });
    
    // Sumar las manuales e iniciales que quedaron
    oldTx.filter(t => t.type !== 'SYSTEM').forEach(t => {
        runningBalance += t.amount;
    });
    
    // 7. Actualizar el Settings con el nuevo balance final
    const settingsRef = doc(db, `consortiums/${consortiumId}/settings`, 'general');
    batch.set(settingsRef, { reserveFundBalance: runningBalance }, { merge: true });
