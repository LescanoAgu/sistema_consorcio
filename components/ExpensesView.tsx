...
const ExpensesView: React.FC<ExpensesViewProps> = ({ expenses, setExpenses, reserveBalance, consortiumId, units }) => {
  ...
  const [newExpense, setNewExpense] = useState<Partial<Expense>>({
    description: '', amount: 0, date: getLocalIsoDate(),
  ...
  const handleSave = async () => {
    ...
    try {
        const expenseData = {
            description: newExpense.description || '', amount: amount, date: newExpense.date || getLocalIsoDate(),
        ...
        if (editingId) {
            ...
            if (originalExpense?.distributionType === ExpenseDistributionType.FROM_RESERVE) {
              ...
              for (const row of (data as any[])) {
                  ...
                  if (desc && amount > 0) {
                      const newExp = {
                          ...
                          date: row['Fecha'] ? getLocalIsoDate(new Date(row['Fecha'])) : getLocalIsoDate(),
  ...
  const handleExportExcel = () => {
      const data = expenses.map(e => ({
          Fecha: formatLocalDate(e.date), Rubro: e.itemCategory, Descripción: e.description,
  ...
  return (
    <div className="space-y-6">
      ...
      {isFormOpen && (() => {
          const isEditingReserve = editingId && expenses.find(e => e.id === editingId)?.distributionType === ExpenseDistributionType.FROM_RESERVE;
          ...
          return (
            <div className="bg-white p-6 rounded-xl shadow-md border border-indigo-100 animate-fade-in relative">
              ...
              {isEditingReserve && (
                  <div className="mb-4 bg-amber-50 border border-amber-200 text-amber-800 p-3 rounded-lg text-sm flex gap-2">
                      <span>⚠️</span>
                      <p>Este gasto fue descontado del Fondo de Reserva. Para mantener la consistencia contable, no puedes modificar su monto o tipo. Si necesitas hacerlo, elimínalo y crea uno nuevo.</p>
                  </div>
              )}
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" className={`w-full p-2 border rounded outline-none font-bold text-indigo-700 ${isEditingReserve ? 'bg-slate-100' : ''}`} disabled={!!isEditingReserve} value={newExpense.amount||''} onChange={e => setNewExpense({ ...newExpense, amount: parseFloat(e.target.value)||0 })} />
                    </div>
                </div>
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="flex flex-col justify-between">
                        <div>
                            ...
                            <select className={`w-full p-2 border border-slate-200 rounded outline-none bg-white font-medium text-slate-700 ${isEditingReserve ? 'bg-slate-100' : ''}`} disabled={!!isEditingReserve} value={newExpense.distributionType} onChange={e => setNewExpense({ ...newExpense, distributionType: e.target.value as any })}>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {gastosAgrupados[cat].map(e => (
                                <tr key={e.id} className="hover:bg-slate-50 group transition-colors">
                                    <td className="px-6 py-4 text-slate-500 w-32">{formatLocalDate(e.date)}</td>
